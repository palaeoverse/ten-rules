...
# Download occurrences from Paleobiology Database, these are occurrences for Rupelian (early Oligocene) crocodilians
url <- "https://paleobiodb.org/data1.2/occs/list.csv?datainfo&rowcount&base_name=Crocodylomorpha&interval=Rupelian&pgm=gplates,scotese,seton&show=full"
occs <- read.csv(url, stringsAsFactors = FALSE, skip = 19)  # Skip first 19 rows

# Select relevant columns
occs <- occs %>% select(accepted_name, identified_name, family, lng, lat)
...
occs <- occs %>% distinct(lng, lat, .keep_all = TRUE)
...
cleaned_occurrences <- occs %>% filter(!duplicate_flag)
...
ggplot(data = world) +
  ...
  geom_point(data = cleaned_occurrences, aes(x = lng, y = lat, color = accepted_name), alpha = 0.7, size = 2) +
  theme_minimal() +
