...
Here we present a full example workflow that complies with the ten rules we propose in the manuscript. The intention is to provide further details and ideas to the concepts outlined in the manuscript, as well as code that can be repurposed by readers.
...
new_data <- read_csv(metadata[5, 2], header = TRUE)
```
...
<!-- I spotted that we have two cc columns (cc and cc.1) which is making us have funky column names? -->
...
<ul>
  <li> Following general guidelines for good coding, such as ample use of spacing to help human readability </li>
  <li> Having a clear, overarching code structure (such as subsections, which can be denoted with headings and "#====" notation, and an order which corresponds to the flow of the research methods/results) </li>
  <li> Using logical yet short variable names </li>
  <li> Including comments throughout which explain both *what* and *why* </li>
</ul>
...
Here, the Ypresian is looking pretty suspicious - it seems to have a lot of outliers. Let's plot the Ypresian occurrences on a palaeogeographic map to investigate further.
...
# Filter occurrences to those identified at species level, then tabulate species names
...
fossils %>% 
  ...
  arrange(early_interval) # Putting the list in order so it's easier to scan for
                          # inconsistencies
